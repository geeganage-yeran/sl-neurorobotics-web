...
public interface ShippingAddressRepository extends JpaRepository<Shipping_address,Long> {
    @Query("SELECT sa FROM Shipping_address sa WHERE sa.street_address = ?1 AND sa.city = ?2 AND sa.state = ?3 AND sa.zipcode = ?4 AND sa.country= ?5")
    List<Shipping_address> findByAddressFields(String streetAddress, String city, String state, String zipcode, String country);
